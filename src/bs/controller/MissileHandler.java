	...
	/* Methods exclusive to CPU Firing Artificial Intelligence */
		...
		public void fireCPUMissile(BsPlayer cpu, BsPlayer user){
			...
			while(!isShotFired){
				...
				}else if(game.getCPU().getRandSearchFails()<game.getCPU().getSearchStandard()){//fire semi-randomly (no need to check for not pursing line?)
		...
		public boolean fireCPUSemiRandomly(int r, int c, BsPlayer cpu, BsPlayer user){//firing randomly with some limitations
			if(cpu.getOffBoard().getSquare(r, c).equals("|__")){//if the space is available
				if(notBorderingAMiss(cpu, r, c)){
					//if the space isn't touching another strike vertically or horizontally and it isn't surrounded by other strikes
					markOffBoard(r, c, cpu, user.getDefBoard());//marks the space hit or miss on the CPU's offensive board
					return true;
				}else{
					int temp = cpu.getRandSearchFails();
					cpu.setRandSearchFails(++temp);
					return false;
				}
		...
		public boolean fireCPURandomly(int r, int c, BsPlayer cpu, BsPlayer user){//firing totally randomly, returns true if successfully marked
			if(cpu.getOffBoard().getSquare(r, c).equals("|__") && !surroundedByMisses(cpu, r, c)){//if the space is available and could be a hit
				markOffBoard(r, c, cpu, user.getDefBoard());//marks the space hit or miss on the CPU's offensive board
				return true;
			}else{//the randomly generated space is already occupied by a strike
				return false;
		...
		private boolean notBorderingAMiss(BsPlayer cpu, int row, int column){
			if(cpu.getOffBoard().getSquare(row+1, column).equals("| O")){
				return false;
			}
			if(cpu.getOffBoard().getSquare(row-1, column).equals("| O")){
				return false;
			}
			if(cpu.getOffBoard().getSquare(row, column+1).equals("| O")){
				return false;
			}
			if(cpu.getOffBoard().getSquare(row, column-1).equals("| O")){
				return false;
		...
		private boolean surroundedByMisses(BsPlayer cpu, int row, int column){
			int missBorder=0;
